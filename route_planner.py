...
Defines the RoutePlanner class, which manages drone fleet initialization, step-by-step
navigation across network hubs using path cost heuristics, capacity limits, and turn outputs.
"""
...
class RoutePlanner():
    ...
    Attributes:
        network_zone (NetworkZone): Network topology containing start/end nodes and hubs.
        mapper (Mapper): Spatial mapping engine holding node costs and connectivity.
    ...
    def __init__(self, network_zone: NetworkZone) -> None:
        """Initializes the RoutePlanner, instantiates drones, and computes routes.

        Args:
            network_zone (NetworkZone): The network graph environment for drone routing.
        """
    ...
    def _create_drones(self) -> list[Drone]:
        """Instantiates all drones assigned to the network and places them at start.
    ...
    def _drone_routes(self) -> None:
        """Simulates iterative movements for all drones until reaching destination.

        Raises:
            RuntimeError: If the iteration count exceeds safety bounds, indicating a deadlock.
        """
    ...
    def _move_drone(
        ...
        Args:
            drone_position (int): Index position of the drone in the active queue.
            drone (Drone): The target drone being moved.
            iteration_route (dict[str, int]): Tracked connection usage for current turn.
    ...
    def _get_connection_name(self, hub_a: str, hub_b: str) -> str:
        ...
        Returns:
            str: Custom connection name if defined, otherwise formatted connection key.
        """
    ...
    def _search_next_node(
        ...
        """Finds valid adjacent neighbor nodes adhering to capacity constraints.

        Args:
            drone_position (int): Current index of the drone within the turn sequence.
            drone (Drone): The drone attempting to move.
            iteration_route (dict[str, int]): Connection passage counts for current turn.

        Returns:
            MapNode: Selected optimal neighbor node, or current node if no moves are available.
        """
        ...
        for neighbor in current_node.neighbors.values():
            if self._is_valid_neighbor(neighbor):
                ...
                if self.mapper.connection_capacity.get(key, 0) > iteration_route.get(key, 0):
    ...
    def _better_node(
        ...
        """Selects the best destination node using cost heuristics and priority counts.

        Args:
            drone_position (int): Position index used for tie-breaking/margin calculations.
    ...
    def _is_valid_neighbor(self, neighbor: MapNode) -> bool:
        ...
        Returns:
            bool: True if the node is accessible and under capacity, False otherwise.
        """
            ...
            for drone in self.drone_list:
                if iteration in drone.route:
                    ...
                    if last_value != value and not (value == self.network_zone.start.name and iteration == 0):
                        ...
                        color_name: str | None = hub.metadata.get('color') if (hub and 'color' in hub.metadata) else None
